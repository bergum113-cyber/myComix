...
**버그 수정 — 수동 북마크 추가 후 목록에 즉시 반영되지 않던 문제(bfcache stale)**
- 뷰어에서 "+"로 북마크 저장(서버엔 저장됨) 후 뒤로가기로 목록(`index.php`)에 돌아오면, **bfcache 복원**이라 옛 목록이 그대로 보였음(복원 시 즐겨찾기만 동기화하고 북마크/자동저장 목록은 갱신 안 함). `save_bookmark()`가 저장 시 `sessionStorage`에 플래그를 남기고, `index.php`의 `pageshow`(bfcache 복원)에서 **그 플래그가 있을 때만** 새로고침하도록 수정(없으면 기존대로 빠르게 즐겨찾기만 동기화 → 불필요한 새로고침 없음). 즉 수동 북마크 추가 시에만 목록이 최신으로 갱신됨. (자동저장 목록은 매 이탈마다 변해 새로고침이 잦아지므로 의도적으로 제외 — 다음 일반 이동 시 갱신.)
...
> 코드 변경은 `config.php`(버전)와 `blank.php`·`login.php`·`viewer.php`·`index.php`·`bookmark.php`에 한정됩니다. v2.4.1의 변경 파일(`index.php`·`function.php`·`thumb.php`·`archive_handler.php`)은 그대로 유지되며, 그 외 파일은 v2.4 원본과 동일합니다. 실서버(iOS·실세션·PC 브라우저) 환경에서의 재현 검증은 배포 후 확인이 필요합니다.
